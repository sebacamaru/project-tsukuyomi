import{c as e,l as t,o as n,s as r,u as i}from"./index-CqwTlPNv.js";import{n as a,t as o}from"./Scene-RZqpEbS7.js";import{t as s}from"./assetRegistry-BWba7ag2.js";var c=`<div class="inventory">
    <div class="container">
...
`,l=class{constructor(e){this.container=typeof e.container==`string`?document.querySelector(e.container):e.container,this.items=e.items||[],this.filteredItems=[...this.items],this.onItemClick=e.onItemClick||null,this.showQuantity=e.showQuantity!==!1,this.emptyMessage=e.emptyMessage||`Sin items`,this.emptyElement=e.emptyElement||null,this.lazyLoadRoot=e.lazyLoadRoot||this.container,this.tooltip=null,this.activeItem=null,this.isMobile=`ontouchstart`in window||navigator.maxTouchPoints>0,this.hideTooltipTimeout=null,this.listeners=[],this.init()}init(){this.createTooltip(),this.setupEvents()}createTooltip(){document.body.insertAdjacentHTML(`beforeend`,`
    ...
    `}setItems(e){this.items=e||[],this.filteredItems=[...this.items],this.render()}filter(e){e===`all`?this.filteredItems=[...this.items]:this.filteredItems=this.items.filter(t=>t.type===e),this.render()}getItemData(e){let t=e.dataset.inventoryId;return this.items.find(e=>String(e.id)===t)}on(e,t,n,r=!1){e.addEventListener(t,n,r),this.listeners.push({element:e,event:t,handler:n,useCapture:r})}destroy(){this.hideTooltipTimeout&&clearTimeout(this.hideTooltipTimeout),this.listeners.forEach(({element:e,event:t,handler:n,useCapture:r})=>{e.removeEventListener(t,n,r)}),this.listeners=[],this.tooltip&&=(this.tooltip.remove(),null),this.activeItem=null}},u=class extends o{constructor(){super(),this.itemGrid=null,this.allItems=[]}async getHTML(){return c}async initUI(){await this.loadInventory(),this.updateGold(),this.allItems=this.normalizeInventory(),this.itemGrid=new l({container:this.$(`#inventoryItems`),items:this.allItems,showQuantity:!0,emptyElement:this.$(`#emptyMessage`),lazyLoadRoot:this.root}),this.itemGrid.render(),this.onClick(`.tab`,(e,t)=>{this.$$(`.tab`).forEach(e=>e.classList.remove(`active`)),t.classList.add(`active`),this.itemGrid.filter(t.dataset.type)})}async loadInventory(){await Promise.all([t.getUserEggs(),e.getUserCandies(),r.getUserStones(),n.getUserChigos()])}normalizeInventory(){let e=[];return(i.eggs||[]).filter(e=>e.status===`inventory`).forEach(t=>{e.push({id:`egg_${t.id}`,item_id:t.id,uuid:t.uuid,type:`egg`,name:t.type_name,label:t.type_label||t.type_name,description:t.type_description||`Un huevo misterioso`,icon:t.icon,quantity:1,price:0,_original:t})}),(i.candies||[]).forEach(t=>{t.quantity>0&&e.push({id:`candy_${t.candy_type_id}`,item_id:t.candy_type_id,type:`candy`,name:t.name,label:t.label||t.name,description:t.description||`Un caramelo para tu chigo`,icon:t.icon,quantity:t.quantity,price:t.price||0,_original:t})}),(i.stones||[]).forEach(t=>{t.quantity>0&&e.push({id:`stone_${t.stone_type_id}`,item_id:t.stone_type_id,type:`stone`,name:t.name,label:t.label||t.name,description:t.description||`Una piedra elemental`,icon:t.icon,quantity:t.quantity,price:t.price||0,_original:t})}),(i.chigos||[]).forEach(t=>{e.push({id:`chigo_${t.id}`,item_id:t.id,uuid:t.uuid,type:`chigo`,name:t.species_name,label:t.nickname||t.species_label||t.species_name,description:`Lv.${t.level||1} - HP:${t.hp} ATK:${t.atk} DEF:${t.def} SPD:${t.spd}`,icon:t.icon,quantity:1,price:0,_original:t})}),e}updateGold(){let e=this.$(`#goldAmount`);e&&(e.textContent=i.gold.toLocaleString())}onExit(){this.itemGrid&&=(this.itemGrid.destroy(),null),super.onExit()}};export{u as InventoryScene};